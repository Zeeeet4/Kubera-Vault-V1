function createAccountForm(account = null) {
    ...
    const banks = DB.getBankEntities();
    ...
    return `
        <form id="accountForm" class="form">
            ...
            <div class="input-group">
                ...
                <select class="select" name="bankName" onchange="var inp=document.getElementById('bankNameCustom');if(inp){inp.style.display=this.value==='__custom__'?'block':'none';if(this.value==='__custom__')inp.focus();}">
                    <option value="">Seleccionar...</option>
                    ${banks.map(b => `
            ...
            <div class="input-group">
                ...
                <small style="color: var(--text-muted); font-size: var(--text-xs);">Este es el balance base. Se calculará automáticamente con entradas y salidas.</small>
            </div>
            <div class="input-group">
                ...
                <div class="color-picker" id="colorPicker">
                    ...
                    `).join('')}
                    <input type="color" class="color-custom" id="customColorPicker" value="${account?.color || '#2563EB'}" style="width: 32px; height: 32px; border: none; cursor: pointer; border-radius: 50%;">
                </div>
...
function createEntryForm(entry = null) {
    const isEdit = !!entry;
    const currency = store.state.settings.currency || 'USD';
    const accounts = store.state.accounts;
    const nomenclatureMode = entry?.nomenclatureMode || 'manual';
    
    // Get saved nomenclature codes for reuse
    const savedCodes = DB.getNomenclatureCodesSync ? DB.getNomenclatureCodesSync() : [];
        ...
        <form id="entryForm" class="form">
            ...
            <div class="input-group">
                ...
                <div style="margin-bottom: 8px;">
                    ...
                    <select class="select" onchange="applyNomenclatureCode(this.value)" style="font-size: var(--text-sm);">
                ...
                <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 8px;">
                    <select class="select" name="nomenclatureMode" id="nomenclatureMode" style="flex: 1;">
                ...
                <div id="nomenclatureFields">
                    ${nomenclatureMode === 'auto-siglas' ? `
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <span class="badge badge-primary" id="autoSiglas" style="font-family: var(--font-mono);">${generateSiglas(entry?.title || '')}</span>
                            <span style="color: var(--text-muted); font-size: var(--text-sm);">-${generateAutoCode(entry)}</span>
                        </div>
                    ` : ''}
                    ${nomenclatureMode === 'hybrid' ? `
                        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                            <span class="badge badge-primary" id="hybridSiglas" style="font-family: var(--font-mono);">${generateSiglas(entry?.title || '')}</span>
                            <input type="text" class="input" name="nomenclatureCode" id="nomenclatureCode" value="${entry?.nomenclatureCode || ''}" placeholder="Código (ej: 01A)" style="width: 80px; font-family: var(--font-mono);">
                            <span style="color: var(--text-muted); font-size: var(--text-sm);">-${generateAutoCode(entry)}</span>
                        </div>
                    ` : ''}
            ...
            <div class="input-group">
                <div class="checkbox-group">
                    <input type="checkbox" class="checkbox" name="recurring" id="recurringCheck" ${entry?.recurring ? 'checked' : ''}>
                    <label for="recurringCheck">Es recurrente</label>
                </div>
            </div>
    ...
    return `
        <form id="expenseForm" class="form">
            ...
            <div class="input-group">
                ...
                <small style="color: var(--text-muted); font-size: var(--text-xs);">Si seleccionas una deuda, el monto se descontará automáticamente del saldo pendiente.</small>
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                <div class="checkbox-group">
                    <input type="checkbox" class="checkbox" name="isInstallment" id="installmentCheck" ${expense?.isInstallment ? 'checked' : ''}>
                    <label for="installmentCheck">Es compra a cuotas</label>
                </div>
            </div>
...
function createDebtForm(debt = null) {
    ...
    if (originalAmount && interestRate && periodicPayment && startDate) {
        ...
        const payment = parseFloat(periodicPayment);
    ...
    return `
        <form id="debtForm" class="form">
            ...
            <div class="input-group">
                ...
                ` : ''}
                <input type="text" class="input" name="nomenclatureCode" id="nomenclatureCode" value="${debt?.nomenclatureCode || ''}" placeholder="Código (ej: DEB-BCP-001)" style="font-family: var(--font-mono); font-size: var(--text-sm);">
            </div>
            <div class="input-group">
...
function createCreditCardForm(card = null) {
    const isEdit = !!card;
    const savedCodes = DB.getNomenclatureCodesSync();
    const cardCodes = savedCodes.filter(c => c.type === 'expense');

    return `
        <form id="creditCardForm" class="form">
            <div class="input-group">
                <label class="input-label">Nombre *</label>
            ...
            <div class="input-group">
                ...
                ` : ''}
                <input type="text" class="input" name="nomenclatureCode" id="nomenclatureCode" value="${card?.nomenclatureCode || ''}" placeholder="Código (ej: TC-VISA-001)" style="font-family: var(--font-mono); font-size: var(--text-sm);">
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <small style="color: var(--text-muted); font-size: var(--text-xs);">Anualidad, seguro, u otros cargos fijos mensuales.</small>
            </div>
            <div class="input-group" style="border-top: 1px solid var(--border-light); padding-top: 16px; margin-top: 8px;">
                ...
                <div id="monthlyPaymentField" style="display: ${(card?.hasMonthlyPayment) ? 'block' : 'none'}; margin-top: 12px;">
                    ...
                    <small style="color: var(--text-muted); font-size: var(--text-xs);">Monto fijo que debes pagar cada mes (independiente del interés).</small>
                </div>
            </div>
            <script>
                document.getElementById('hasMonthlyPayment')?.addEventListener('change', function() {
                    document.getElementById('monthlyPaymentField').style.display = this.checked ? 'block' : 'none';
                });
            </script>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <div class="color-picker" id="cardColorPicker">
                    ${DB.getAccountColors().map(c => `
                        <div class="color-option ${(!card?.color || card?.color === c) ? 'selected' : ''}" data-color="${c}" style="background-color: ${c}"></div>
                    `).join('')}
                    <input type="color" class="color-custom" id="cardCustomColorPicker" value="${card?.color || '#2563EB'}" style="width: 32px; height: 32px; border: none; cursor: pointer; border-radius: 50%;">
                </div>
...
function createInvestmentForm(investment = null) {
    ...
    return `
        <form id="investmentForm" class="form">
            ...
            <div class="input-group">
                ...
                ` : ''}
                <input type="text" class="input" name="nomenclatureCode" id="nomenclatureCode" value="${investment?.nomenclatureCode || ''}" placeholder="Código (ej: INV-DPF-001)" style="font-family: var(--font-mono); font-size: var(--text-sm);">
            </div>
            <div class="input-group">
...
function generateAutoCode(entry) {
    const now = new Date();
